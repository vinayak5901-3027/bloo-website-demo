...
export const products: ProductContent[] = [
  ...
  // ─────────────────────────────────────────────────────── Bloo Vantage (SIEM)
  {
    ...
    problem: {
      ...
      intro:
        'Your SIEM is only as intelligent as your devices are. If the device did not recognize something, it did not emit a log. Disconnected tools mean blind spots. Bloo Vantage closes them all at once.',
      points: [
        { title: 'Log blind spots', description: 'Log-based detection depends on devices emitting logs. If the device did not recognize something, it did not emit a log. NBAD monitors unconditionally at the packet layer  -  volume, flow direction, port distribution and ASN across every hop  -  where data is unconditional.' },
        ...
        { title: 'Fragmented signals', description: 'Four specialists and a bridge call to correlate a single incident. Vantage assembles the correlated signal before it reaches the analyst queue  -  SIEM, UEBA, NBAD and EPM context in one view.' },
      ],
    },
    stats: {
      eyebrow: 'THE NUMBERS',
      ...
      items: [
        { value: '600+', label: 'Active detection workbooks', sub: 'Real-time streaming, continuously updated alongside the threat landscape' },
        { value: '98.4%', label: 'Data compression', sub: 'vs. 55% industry average  -  63 nodes where competitors need 530' },
        { value: '7-Day', label: 'UEBA rolling baseline', sub: 'Baselines retrain continuously  -  persistent drift becomes the new normal' },
    ...
    features: {
      ...
      intro: 'All engines draw from and write back to Bloo Datafabric  -  all logs, all flows, endpoint telemetry, IoT events and cloud telemetry in one unified system of record.',
      items: [
        { icon: 'shield', title: 'SIEM  -  Log-Based Detection at Scale', description: '600+ active detection workbooks in real-time streaming, updated continuously alongside the threat landscape. Uncapped flat-fee licensing with a 14% year-on-year cap. Non-index-based S3 retention  -  keep a year, query a year. Native cloud connectors for Azure, AWS, GCP, Beats, Box, GSuite, o365, Okta, OneLogin, Orca, Prisma, Salesforce and Snowflake.' },
        { icon: 'brain', title: 'UEBA  -  Behavioral Baselines for Users & Entities', description: 'ML anomaly detection across users, hosts, applications, IPs and every entity in the environment. Two model families: Time-Series (Prophet, 1-day hourly forecasting) and Unsupervised (Isolation Forest, 1-hour per-population outlier). Rolling seven-day baseline retraining. Four baseline types: normal behavior, rare behavior (DBSCAN), peer-group comparison and entity baselines. Every model maps to a named threat group carrying a MITRE ATT&CK tactic and technique.' },
        { icon: 'network', title: 'NBAD  -  Network Behavior Anomaly Detection', description: 'Isolation Forest detection at the packet layer across 29 features per NetFlow record. Progressive model promotion pipeline: Base 10 min → 30 min → 1 hr → 1 day → 15 days → 1 month. Noise floor drops with each tier; highest-confidence model is always live. Your SIEM is only as intelligent as your devices  -  NBAD monitors unconditionally where logs are silent.' },
        { icon: 'server', title: 'EPM  -  Endpoint Protection Management', description: 'Lightweight osquery agent for Linux and Windows. Continuous interval-based collection: running processes, active network connections, logged-in users, file activity, installed packages and user sessions. Online/offline heartbeat every 5 seconds. Token-based fleet enrollment via Ansible. Live Response queries any enrolled endpoint in near-real-time. YARA rule execution on demand  -  supply a rule, select the endpoint, review results in the Vantage console.' },
        { icon: 'bot', title: 'B Copilot  -  Embedded AI Assistant', description: 'Available platform-wide as a full-page interface and as a side panel within signals, workbooks, baselines and model pages. Understands the Bloo data schema, entity model, osquery framework and product documentation. Analysts work in natural language  -  "List all users with failed logins in the last 7 days", "Give me the rare behavior for user X", "Show me the running processes on this host." Routes each prompt to the correct engine and returns evidence-backed answers.' },
        { icon: 'activity', title: 'Signal Management & MITRE ATT&CK', description: 'A single unified queue for signals from all four engines. Signal state (New / Processing / Waiting / Closed) and triage outcome (False Positive / Malicious / Remediated) attach directly to every record  -  an operational audit trail without extra work. Cross-source view: SIEM, UEBA and NBAD volumes side-by-side. Platform-computed Mean Time to Triage and Remediate. Full ATT&CK framework rendered with workbook coverage per tactic and technique and last-signal timestamps  -  evidence a CISO can cite without pulling a separate report.' },
      ],
    },
    workflow: {
      ...
      steps: [
        { title: 'Ingest everything', description: 'NetFlow V1/V5/V10, IPFIX, sFlow, syslog, Zeek/Suricata, cloud audit trails, endpoint telemetry, IoT syslog  -  any timestamped log, unconditionally.' },
        { title: 'SIEM detects', description: '600+ streaming workbooks evaluate log-based signals in real time with BQL, Python blocks and AI Blocks for no-code ML and outlier detection.' },
    ...
    capabilities: {
      ...
      rows: [
        { capability: 'Detection engines', description: 'SIEM, UEBA, NBAD (Beta), EPM (Beta)  -  all drawing from and writing back to Bloo Datafabric.' },
        ...
        { capability: 'Connectors', description: 'Native cloud connectors for Azure, AWS, GCP and application providers: Beats, Box, GSuite, o365, Okta, OneLogin, Orca, Prisma, Salesforce, Snowflake.' },
        ...
        { capability: 'Licensing', description: 'Uncapped flat-fee with a 14% year-on-year cap. Ingest 2 TB/day at year two  -  the cost does not change. Finance gets complete predictability; infrastructure has no incentive to suppress data collection.' },
        { capability: 'UEBA models', description: 'Time-Series (Prophet, 1-day hourly forecasting) and Unsupervised (Isolation Forest, 1-hour per-population outlier). Baselines: normal behavior, rare behavior (DBSCAN), peer-group comparison, entity baselines. Threat grouping maps to MITRE ATT&CK tactic, technique, risk score and investigation tips.' },
        { capability: 'NBAD models', description: 'Isolation Forest across 29 NetFlow features. Model promotion pipeline: Base 10 min → 30 min → 1 hr → 1 day → 15 days → 1 month. Highest-confidence model always live; noise floor drops with each tier.' },
        { capability: 'EPM collection', description: 'Running processes, active network connections, logged-in users, file activity, installed packages, user sessions. Online/offline heartbeat every 5 seconds. Token-based fleet enrollment via Ansible.' },
        { capability: 'EPM Live Response', description: 'Live API pull every 10 minutes. Queries any enrolled, online endpoint and returns results in near-real-time. B Copilot understands osquery schema natively  -  analysts work in natural language.' },
        { capability: 'YARA rules', description: 'Execute on demand against any enrolled endpoint. Supply a rule, select the target endpoint; results return to the Vantage console for review and signal creation.' },
        ...
        { capability: 'Dashboards & reports', description: 'Pre-built dashboards covering endpoint fleet health, NBAD protocol breakdown, UEBA entity anomaly summary and SIEM signal trends. Reports are scheduled or run on demand and downloadable as PDF.' },
    ...
    highlights: {
      ...
      items: [
        'SIEM, UEBA, NBAD and EPM on one data fabric  -  no gaps, no stitching',
        ...
        'NBAD: Isolation Forest across 29 NetFlow features, progressive model promotion',
        ...
        'Native connectors: Azure, AWS, GCP, Okta, Salesforce, Snowflake and more',
      ],
    },
    faqs: [
      { q: 'What are the detection engines in Bloo Vantage?', a: 'SIEM (log-based detection with 600+ workbooks), UEBA (user and entity behavioral baselines with ML), NBAD (network behavior anomaly detection at the packet layer via Isolation Forest across 29 NetFlow features), and EPM (endpoint protection management via lightweight osquery agent). All engines draw from and write back to Bloo Datafabric.' },
      { q: 'How does UEBA differ from SIEM detection?', a: 'SIEM detects against known workbooks and rules applied to log data. UEBA applies ML to behavioral baselines for every user and entity  -  separating persistent change (new normal) from transient drift (credential theft). Two model families: Time-Series (Prophet, hourly forecasting) and Unsupervised (Isolation Forest, per-population outlier). Baselines retrain on a rolling seven-day window.' },
      { q: 'Why does NBAD matter if we already have a SIEM?', a: 'Your SIEM is only as intelligent as your devices are. If a device did not recognize an event, it did not emit a log. NBAD monitors unconditionally at the packet layer  -  volume, flow direction, port distribution and ASN across every hop  -  closing the gap where logs are silent. A 10-minute model is noisier than a 30-day model; NBAD solves this by progressively merging short-window models into longer-horizon models as they accumulate.' },
      { q: 'How does the flat-fee licensing work?', a: 'Bloo uses an uncapped flat annual fee with a 14% year-on-year cap. Ingest 2 TB/day at year two and the cost does not change. Finance gets complete predictability; infrastructure has no incentive to suppress data collection. Competitors price by volume: 30 GB/day costs X, 60 GB/day costs 2X.' },
      ...
      { q: 'What is EPM Live Response?', a: 'Live Response sends queries directly to the osquery agent on any enrolled, online endpoint and returns results in near-real-time. B Copilot understands the osquery schema natively  -  analysts work in natural language without knowing table names or column structure. Queries return: running processes, active network connections, logged-in users, file activity and installed packages.' },
      { q: 'What log and telemetry sources does Vantage support?', a: 'NetFlow V1/V5/V10, IPFIX, sFlow, Syslog, Zeek/Suricata, AWS CloudTrail, Azure Activity, GCP Audit, Okta (roadmap), Microsoft Entra (roadmap), Linux & Windows EPM, IoT Syslog, and any timestamped log.' },
    ],
    related: ['datafabric', 'synthai', 'crucible'],
    meta: {
      title: 'Bloo Vantage | Unified Threat Detection  -  SIEM, UEBA, NBAD, EPM',
      description: 'Bloo Vantage unifies four detection engines  -  SIEM, UEBA, NBAD and EPM  -  on one data fabric. 600+ workbooks, 98.4% compression, 365-day hot retention, B Copilot AI assistant.',
      keyword: 'unified threat detection SIEM UEBA NBAD EPM endpoint telemetry',
    },
    pillar: 'Reasoning',
  },
